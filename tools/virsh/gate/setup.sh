#!/usr/bin/env bash

set -ex
set -o pipefail

SCRIPT_DIR=$(realpath $(dirname $0))
WORKSPACE=${WORKSPACE:-$(realpath $SCRIPT_DIR/../../..)}

sudo DEBIAN_FRONTEND=noninteractive apt install -y --no-install-recommends \
    curl \
    docker.io \
    genisoimage \
    libvirt-bin \
    virtinst

for grp in docker libvirtd; do
    if ! groups | grep $grp; then
        sudo adduser `id -un` $grp
    fi
done

if [ "xY" != "x$(cat /sys/module/kvm_intel/parameters/nested)" ]; then
    sudo modprobe -r kvm_intel
    sudo modprobe kvm_intel nested=1
    echo "options kvm-intel nested=1" | sudo tee /etc/modprobe.d/kvm-intel.conf
fi

if !  sudo virt-host-validate; then
    echo 'GRUB_CMDLINE_LINUX_DEFAULT="${GRUB_CMDLINE_LINUX_DEFAULT} intel_iommu=on"' | sudo tee /etc/defaults/grub
fi
# XXX add reboot trigger in groovy
lscpu
if ! grep vmx /proc/cpuinfo > /dev/null; then
    echo ** VMX not enabled! **
    echo ** VMX not enabled! **
    echo ** VMX not enabled! **
fi

SSH_CONFIG_DIR=$WORKSPACE/tools/virsh/config-ssh
mkdir -p ${SSH_CONFIG_DIR}

if [ ! -s ${SSH_CONFIG_DIR}/id_rsa ]; then
    ssh-keygen -N '' -f ${SSH_CONFIG_DIR}/id_rsa
fi

if [ ! -s ${SSH_CONFIG_DIR}/config ]; then
    env -i \
        SSH_CONFIG_DIR=$SSH_CONFIG_DIR \
        envsubst < $WORKSPACE/tools/virsh/templates/ssh-config.sub > ${SSH_CONFIG_DIR}/config
fi
