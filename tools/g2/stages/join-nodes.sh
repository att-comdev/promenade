...
render_validate_body() {
    if [[ ${USE_DECKHAND} == 1 ]]; then
        JSON="{\"rel\":\"design\",\"href\":\"deckhand%2Bhttp://deckhand-int.ucp.svc.cluster.local:9000/api/v1.0/revisions/${DECKHAND_REVISION}/rendered-documents\",\"type\":\"application/x-yaml\"}"
    else
...
for NAME in "${NODES[@]}"; do
    ...
    ssh_cmd "${VIA}" curl -v "${CURL_ARGS[@]}" -X POST -d $(render_validate_body) "${BASE_PROM_URL}/api/v1.0/validatedesign"
