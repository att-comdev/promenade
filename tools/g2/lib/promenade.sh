...
promenade_api_health() {
    ...
    ssh_cmd "${VIA}" docker run --rm -t -w /target -v ${BUILD_DIR}:/target \
        "registry:5000/${IMAGE_PROMENADE}" promenade check-health "${ARGS}"
}
...
promenade_api_validatedesign() {
    ...
    ssh_cmd "${VIA}" docker run --rm -t -w /target -v ${BUILD_DIR}:/target \
        "registry:5000/${IMAGE_PROMENADE}" promenade validatedesign --href "${REF_URL}" "${ARGS}"
}
...
promenade_api_join_scripts() {
    ...
    log Running join script command
    ssh_cmd "${VIA}" docker run --rm -t -w /target -v ${BUILD_DIR}:/target \
        "registry:5000/${IMAGE_PROMENADE}" promenade join-scripts "${ARGS}" > "${SCRIPT_DIR}/join-${NAME}.sh"
    log Chmoding script
    chmod 755 "${SCRIPT_DIR}/join-${NAME}.sh"
    log Sending script to node
