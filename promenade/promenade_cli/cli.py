...
def check_health(ctx, os_project_domain_name, os_user_domain_name,
    ...
    try:
        if all([
            ...
            prom_client = get_client(ctx, url=url)
...
def join_scripts(ctx, os_project_domain_name, os_user_domain_name,
                 os_project_name, os_username, os_password, os_auth_url, token,
                 url, hostname, ip, design_ref, leave_kubectl, dynamic_labels,
                 static_labels):
    debug = _debug()
    try:
        if all([
        ...
        click.echo(
            json.dumps(
                client_helper.JoinScripts(
                    prom_client, hostname, ip, design_ref, leave_kubectl,
                    dynamic_labels, static_labels).invoke()))
    except exceptions.PromenadeException as e:
        e.display(debug=debug)
        sys.exit(e.EXIT_CODE)
...
def get_client(ctx, url=None, token=None, keystone_env=None):
    ...
    if not url:
        try:
            ...
            LOG.debug(str(e))
            LOG.debug("Promenade CLI requires the URL of the Promenade API"
                      " server or a Keystone server.")
            raise exceptions.ApiError(title="Missing URL")
