...
class PromenadeSession(object):
    ...
    :param string host: The Promenade server hostname or IP
    :param int port: (optional) The service port appended if specified
    :param function auth_gen: Callable that will generate a list of
           authentication header names and values (2 part tuple)
    ...
    def get(self, endpoint, query=None):
        ...
        :param string endpoint: The URL string following the hostname and API
                                prefix
        ...
        while True:
            resp = self.__session.get(
                self.base_url + endpoint, params=query, timeout=60)
    ...
    def post(self, endpoint, query=None, body=None, data=None):
        """
        Send a POST request to Promenade. If both body and data are specified,
        body will will be used.
        :param string endpoint: The URL string following the hostname and
                                API prefix
        :param dict query: A dict of k, v parameters to add to the query string
        :param string body: A string to use as the request body. Will be
                            treated as raw
        :param data: Something json.dumps(s) can serialize. Result will be used
                     as the request body
        :return: A requests.Response object
        """
        auth_refresh = False
        while True:
            self.logger.debug("Sending POST with promenade_client session")
            if body is not None:
                self.logger.debug(
...
class KeystoneClient(object):
    ...
    def get_endpoint(endpoint, ks_sess=None, auth_info=None):
        ...
        :param ks_sess: A keystone session to use for accessing endpoint
                        catalogue
        :param auth_info: Authentication info to use for building a token if a
                          ``ks_sess`` is not specified
    ...
    def get_token(ks_sess=None, auth_info=None):
        ...
        :param auth_info: dictionary of information required to generate a
                          keystone token
        """
        if ks_sess is None:
            ks_sess = KeystoneClient.get_ks_session(**auth_info)
