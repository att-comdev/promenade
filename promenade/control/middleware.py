...
class NoAuthMiddleware(object):
    ...
    def process_request(self, req, resp):
        ctx = req.context
        serv_status = req.get_header('X-SERVICE-IDENTITY-STATUS')

        service = True

        if serv_status is not 'Confirmed':
            if serv_status is None:
                req.headers['X-IDENTITY-STATUS'] = 'Confirmed'
                service = False
            else:
                req.headers['X-SERVICE-IDENTITY-STATUS'] = 'Confirmed'
        ...
        ctx.user_id = req.get_header(
            'X-SERVICE-USER-ID') if service else req.get_header('X-USER-ID')
        ...
        self.logger.debug('Request from noauth')
