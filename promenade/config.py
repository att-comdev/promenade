...
class Configuration:
    ...
    def extract_genesis_config(self):
        ...
        for document in self.documents:
            if document['schema'] != 'promenade/KubernetesNode/v1':
                ...
                LOG.debug('Excluding schema=%s metadata.name=%s', document['schema'],
                          _mg(document, 'name'))
        return Configuration(debug=self.debug, documents=documents, substitute=False)
    ...
    def _get_path(self, path):
        ...
        if document:
            return _extract(document['data'], jsonpath)
