...
function wait_for_node_ready {
    ...
    log Waiting $SECONDS seconds for $NODE_NAME to be ready.
    ...
    log Waiting $SECONDS seconds for specified DNS queries to work:
    ...
    while true; do
        SUCCESS=1
        for DOMAIN in ${REQUIRED_DNS_LOOKUPS[*]}; do
            if ! (dig $DOMAIN @{{ config['KubernetesNetwork:dns.service_ip'] }} > /dev/null 2> /dev/nul); then
                SUCCESS=0
