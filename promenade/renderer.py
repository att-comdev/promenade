...
def build_tarball_from_roles(config, *, roles):
...
def insert_charts_into_bundler(bundler):
    for root, _dirnames, filenames in os.walk('/promenade/charts', followlinks=True):
        for source_filename in filenames:
            source_path = os.path.join(root, source_filename)
            destination_path = os.path.join(
                    'etc/kubernetes/armada/assets/charts',
                    os.path.relpath(source_path, '/promenade/charts'))
            stat = os.stat(source_path)
            LOG.debug('Copying asset file %s (mode=%o)', source_path, stat.st_mode)
            with open(source_path) as f:
                bundler.add(path=destination_path, data=f.read(), mode=stat.st_mode)
...
def render_template_into_bundler(*, bundler, config, destination_path, source_path, mode):
    env = _build_env()
...
def _build_env():
    ...
    env.filters['fill_initial_etcd_cluster'] = _fill_initial_etcd_cluster
...
def _fill_initial_etcd_cluster(hostname):
    return ','.join([
        '%s=https://kubernetes:2380' % hostname,
        'auxiliary-etcd-0=https://kubernetes:12380',
        'auxiliary-etcd-1=https://kubernetes:22380',
    ])
...
def _fill_no_proxy(network_config):
    ...
    if proxy:
        additional = network_config.get('proxy', {}).get('additional_no_proxy', [])
        if additional:
            return ','.join(additional) + ',' + _default_no_proxy(network_config)
        else:
